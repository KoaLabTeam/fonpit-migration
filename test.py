...
from sqlalchemy import Table, Column, Integer, String, BigInteger, DateTime, Sequence, ForeignKey, Text, UniqueConstraint, join
from sqlalchemy.orm import sessionmaker, relationship, backref, column_property


engine = create_engine(
    'mysql+pymysql://root:root@192.168.95.100:4022/local', echo=True)
...
class User(Base):
    ...
    def __repr__(self):
        return "<WpUser(login='%s', email='%s', display_name='%s')>" % (self.user_login, self.user_email, self.display_name)
...
class Comment(Base):
    ...
    comment_parent = Column(Integer, ForeignKey(
        'wp_comments.comment_ID'), nullable=False)
    ...
    likes = relationship('ULikeComments', back_populates='comment')
